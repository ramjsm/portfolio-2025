...
import { getCocreationBySlug, getNextCocreationBySlug, getPreviousCocreationBySlug, getProjectCategoryLabel, getProjectsByCategory } from '../router/cocreations'
...
export function ProjectTemplate({ category }) {
    ...
    const list = ['/cocreations/synthara/synthara1.jpg', '/cocreations/synthara/synthara2.jpg', '/cocreations/synthara/synthara3.png']
    return (
        <article className="w-full relative">            
            <div className="relative flex w-full min-h-dvh items-center">
                <div className="absolute transform -translate-x-1/2 -translate-y-1/2 top-[50%] left-[50%] flex flex-col items-center gap-4 z-10 mix-blend-difference">
                    <h1 className="header font-syne uppercase text-7xl drop-shadow-xl/50 text-transparent text-stroke-gray-100 text-stroke-1 whitespace-nowrap">{concreationData.title}</h1>
                </div>
            ...
            <div className="relative flex w-full mb-10">
                    <div className="info-wrapper flex flex-col flex-1 gap-4">
                        {concreationData.info.map((info) => <Info key={info.header} header={info.header} list={info.list} />)}
                    </div>
                    <div className="flex-3 text-xl font-[100] flex flex-col gap-5">{concreationData.intro}</div>
            </div>
            <div className="my-20">
                {concreationData.body}
            </div>
            {concreationData.credits && (<ul className="text-center font-[100] mb-20">
                {concreationData.credits.map((listItem, index) => <li key={index} className="text-xl">{listItem}</li>)}
            </ul>)
            }
                ...
                <div className="flex flex-1 items-center justify-center gap-3 font-pp-neue-montreal text-sm">
                    {projects.map((cocreation, index) => 
                        <Link 
                            ...
                            className={ concreationData.slug === cocreation.slug ? 'text-xl' : 'opacity-50'}
                        >
                            {`/0${index + 1 }`}
                        </Link>
                    )}
